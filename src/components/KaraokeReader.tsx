...
// Docs share link pasted into a .txt), we show the links.
  ...
  return (
    <aside className="paper rounded-sm p-5 h-full flex flex-col">
      <div className="flex items-center justify-between mb-2">
        <div>
          ...
          <h3 className="font-display text-xl inline-flex items-center gap-2">
            <Music2 className="h-4 w-4 text-primary" /> Karaoke Script
          </h3>
        </div>
      ...
      {raw && !extractIfLinksOnly(raw) && timedLines.length > 0 && (
        <div
          ref={containerRef}
          className="flex-1 overflow-y-auto pr-2 space-y-2 max-h-[60vh] min-h-[300px]"
        >
          {timedLines.map((l, i) => {
            ...
            return (
              <button
                ...
                className={[
                  "block w-full text-left font-display leading-snug transition-all px-2 py-1 rounded-sm",
                  isActive
                    ? "text-primary text-2xl md:text-3xl scale-[1.02] origin-left"
                    : isPast
                      ? "text-muted-foreground/60 text-base"
                      : "text-foreground/80 text-base hover:text-foreground hover:bg-secondary/40",
                ].join(" ")}
              >
                {l.text}
              </button>
